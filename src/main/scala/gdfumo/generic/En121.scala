...
trait En121[T] {
  def encode(data: T, nameHList: HList): (List[(String, String)], Int)
}
...
object En121 extends En121Impl {
  ...
  implicit def hlistAppend[B <: HList](implicit e: En121[B]): En121[Option[Json] :: B] = new En121[Option[Json] :: B] {
    override def encode(data: Option[Json] :: B, nameHList: HList): (List[(String, String)], Int) = {
      println(data.head)
      ...
      ((nameHList.asInstanceOf[Symbol :: HList].head.name, value._1) :: tailEncode._1, tailEncode._2 + value._2)
    }
  }

  implicit val hlistZero: En121[HNil] = new En121[HNil] {
    override def encode(data: HNil, nameHList: HList): (List[(String, String)], Int) = (List.empty, 0)
  }
  ...
  def derived[T, H <: HList](implicit
    ...
    val enc1 = new En121[T] {
      def encode(data: T, nameHList: HList): (List[(String, String)], Int) = en121.encode(aa.to(data), nameHList)
    }
    (enc1, bb.apply())
  }
...
trait En121Impl {
  implicit def hlistAppendLow2[A, B <: HList](implicit e: En121[B]): En121[A :: B] = new En121[A :: B] {
    override def encode(data: A :: B, nameHList: HList): (List[(String, String)], Int) =
      e.encode(data.tail, nameHList.asInstanceOf[Any :: HList].tail)
  }
}
